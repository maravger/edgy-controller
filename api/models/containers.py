...
class Container(models.Model):
    ...
    def predict_next_rr(self, sampling_interval):
        ...
        logger.info("Predicting for Container serving App: " + str(self.app_id))
        logger.info(self.prev_subm + self.prev_rej)
        prev_real_rr = float(self.prev_subm + self.prev_rej) / sampling_interval, 2
        logger.info("Previous Real Request Rate: " + str(prev_real_rr))
        s = alpha * prev_real_rr + (1 - alpha) * (self.s - self.b)
        logger.info("s = " + str(s))
        self.b = v * (s - self.s) + (1 - v) * self.b
        logger.info("b = " + str(self.b))
        self.s = round(s + self.b, 2)
        logger.info("Predicted Request Rate: " + str(self.s) + "\n")
        self.next_predicted_rr = self.s

    def calc_cpu_usg(self):
        self.avg_cpu = self.accu_cpu / self.ticks

    def print_logs(self, start_time):
        elapsed_time = time.time() - start_time
        # Get an instance of a logger
        logger = logging.getLogger(__name__)  # type: Logger
        logger.info("Time: {0} | Container: {1} | Average CPU Usage: {2} | Average Interval Response Time: {3} | " +
                    "PEs allocated: {4} | Requests Submitted: {5} | Requests Finished: {6} | Requests Rejected: {7} | " +
                    "Operating Point: {8}").format(elapsed_time, self.cont_id, self.avg_cpu, self.prev_art, self.next_pes, self.prev_subm,
                                                   self.prev_fin, self.prev_rej, self.op_point)
